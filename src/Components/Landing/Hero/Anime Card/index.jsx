...
const index = ({ list }) => {
    return (
        <div className='flex flex-col justify-between sm:flex-row flex-wrap'>
            {list.slice(0, 10).map((anime, i) => {
                return (
                    <div className='sm:max-w-[211px] w-full px-2 mb-6 cursor-pointer' key={anime + i}>
                        <div className='relative'>
                            <img src={anime.images.jpg.large_image_url} draggable={false} className='sm:h-[325px] sm:w-auto h-[300px] w-full object-cover rounded-lg mb-2' alt="" />
                        ...
                        <div className='flex flex-1 flex-wrap'>
                            {anime.genres.map((genre, i) => {
                                return (
                                    <div className='font-[500] rounded-xl px-2 leading-none py-1 mr-2 mb-1 text-[10px] bg-[#00000056] text-white dark:bg-[#ffffff69] transition duration-[0.7s]' key={genre + i}>
                                        {genre.name}
                                    </div>
                                );
                            })}
